...
public class TerrainProvider {
    ...
    private static <C, I extends ToFloatFunction<C>> CubicSpline<C, I> buildMountainRidgeSplineWithPoints(I $$0, float $$1, boolean $$2, ToFloatFunction<Float> $$3) {
        ...
        if (-0.65f < $$10 && $$10 < 1.0f) {
            ...
            if ($$2) {
                $$4.addPoint(-1.0f, Math.max((float)0.2f, (float)$$7));
    ...
    private static float mountainContinentalness(float $$0, float $$1, float $$2) {
        ...
        float $$8 = $$7 * $$5 - $$6;
        if ($$0 < $$2) {
            return Math.max((float)$$8, (float)-0.2222f);
        }
        return Math.max((float)$$8, (float)0.0f);
    }
    ...
    private static <C, I extends ToFloatFunction<C>> CubicSpline<C, I> ridgeSpline(I $$0, float $$1, float $$2, float $$3, float $$4, float $$5, float $$6, ToFloatFunction<Float> $$7) {
        float $$8 = Math.max((float)(0.5f * ($$2 - $$1)), (float)$$6);
        float $$9 = 5.0f * ($$3 - $$2);
        return CubicSpline.builder($$0, $$7).addPoint(-1.0f, $$1, $$8).addPoint(-0.4f, $$2, Math.min((float)$$8, (float)$$9)).addPoint(0.0f, $$3, $$9).addPoint(0.4f, $$4, 2.0f * ($$4 - $$3)).addPoint(1.0f, $$5, 0.7f * ($$5 - $$4)).build();
    }
}