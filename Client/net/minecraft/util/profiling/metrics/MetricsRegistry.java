...
public class MetricsRegistry {
    ...
    public void add(ProfilerMeasured $$0) {
        this.measuredInstances.put((Object)$$0, null);
    }

    public List<MetricSampler> getRegisteredSamplers() {
        Map $$02 = (Map)this.measuredInstances.keySet().stream().flatMap($$0 -> $$0.profiledMetrics().stream()).collect(Collectors.groupingBy(MetricSampler::getName));
        return MetricsRegistry.aggregateDuplicates((Map<String, List<MetricSampler>>)$$02);
    }

    private static List<MetricSampler> aggregateDuplicates(Map<String, List<MetricSampler>> $$02) {
        return (List)$$02.entrySet().stream().map($$0 -> {
            String $$1 = (String)$$0.getKey();
            List $$2 = (List)$$0.getValue();
            return $$2.size() > 1 ? new AggregatedMetricSampler($$1, (List<MetricSampler>)$$2) : (MetricSampler)$$2.get(0);
    ...
    extends MetricSampler {
        ...
        AggregatedMetricSampler(String $$0, List<MetricSampler> $$1) {
            super($$0, ((MetricSampler)$$1.get(0)).getCategory(), () -> AggregatedMetricSampler.averageValueFromDelegates($$1), () -> AggregatedMetricSampler.beforeTick($$1), AggregatedMetricSampler.thresholdTest($$1));
            this.delegates = $$1;
        }
        ...
        @Override
        public int hashCode() {
            return Objects.hash((Object[])new Object[]{super.hashCode(), this.delegates});
        }
    }
}