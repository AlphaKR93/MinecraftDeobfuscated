...
public class FlatLevelGeneratorSettings {
    ...
    public static final Codec<FlatLevelGeneratorSettings> CODEC = RecordCodecBuilder.create($$02 -> $$02.group((App)RegistryCodecs.homogeneousList(Registries.STRUCTURE_SET).optionalFieldOf("structure_overrides").forGetter($$0 -> $$0.structureOverrides), (App)FlatLayerInfo.CODEC.listOf().fieldOf("layers").forGetter(FlatLevelGeneratorSettings::getLayersInfo), (App)Codec.BOOL.fieldOf("lakes").orElse((Object)false).forGetter($$0 -> $$0.addLakes), (App)Codec.BOOL.fieldOf("features").orElse((Object)false).forGetter($$0 -> $$0.decoration), (App)Biome.CODEC.optionalFieldOf("biome").orElseGet(Optional::empty).forGetter($$0 -> Optional.of($$0.biome)), RegistryOps.retrieveElement(Biomes.PLAINS), RegistryOps.retrieveElement(MiscOverworldPlacements.LAKE_LAVA_UNDERGROUND), RegistryOps.retrieveElement(MiscOverworldPlacements.LAKE_LAVA_SURFACE)).apply((Applicative)$$02, FlatLevelGeneratorSettings::new)).comapFlatMap(FlatLevelGeneratorSettings::validateHeight, Function.identity()).stable();
    ...
    private static DataResult<FlatLevelGeneratorSettings> validateHeight(FlatLevelGeneratorSettings $$0) {
        ...
        if ($$1 > DimensionType.Y_SIZE) {
            return DataResult.error((String)("Sum of layer heights is > " + DimensionType.Y_SIZE), (Object)$$0);
        }
    ...
    private FlatLevelGeneratorSettings(Optional<HolderSet<StructureSet>> $$0, List<FlatLayerInfo> $$1, boolean $$2, boolean $$3, Optional<Holder<Biome>> $$4, Holder.Reference<Biome> $$5, Holder<PlacedFeature> $$6, Holder<PlacedFeature> $$7) {
        this($$0, FlatLevelGeneratorSettings.getBiome($$4, $$5), (List<Holder<PlacedFeature>>)List.of($$6, $$7));
        if ($$2) {
            this.setAddLakes();
        }
        if ($$3) {
    ...
    public FlatLevelGeneratorSettings withBiomeAndLayers(List<FlatLayerInfo> $$0, Optional<HolderSet<StructureSet>> $$1, Holder<Biome> $$2) {
        ...
        for (FlatLayerInfo $$4 : $$0) {
            $$3.layersInfo.add((Object)new FlatLayerInfo($$4.getHeight(), $$4.getBlockState().getBlock()));
            $$3.updateLayers();
        }
        if (this.decoration) {
            $$3.setDecoration();
    ...
    public BiomeGenerationSettings adjustGenerationSettings(Holder<Biome> $$0) {
        ...
        if (this.addLakes) {
            for (Holder $$3 : this.lakes) {
                $$2.addFeature(GenerationStep.Decoration.LAKES, (Holder<PlacedFeature>)$$3);
            }
        }
        ...
        if ($$4) {
            ...
            for (int $$6 = 0; $$6 < $$5.size(); ++$$6) {
                ...
                HolderSet $$7 = (HolderSet)$$5.get($$6);
                for (Holder $$8 : $$7) {
                    $$2.addFeature($$6, (Holder<PlacedFeature>)$$8);
                }
            }
        }
        ...
        for (int $$10 = 0; $$10 < $$9.size(); ++$$10) {
            ...
            if (Heightmap.Types.MOTION_BLOCKING.isOpaque().test((Object)$$11)) continue;
            $$9.set($$10, null);
            $$2.addFeature(GenerationStep.Decoration.TOP_LAYER_MODIFICATION, PlacementUtils.inlinePlaced(Feature.FILL_LAYER, new LayerConfiguration($$10, $$11), new PlacementModifier[0]));
        }
        return $$2.build();
    }
    ...
    public void updateLayers() {
        ...
        for (FlatLayerInfo $$02 : this.layersInfo) {
            for (int $$1 = 0; $$1 < $$02.getHeight(); ++$$1) {
                this.layers.add((Object)$$02.getBlockState());
            }
        }
    ...
    public static FlatLevelGeneratorSettings getDefault(HolderGetter<Biome> $$0, HolderGetter<StructureSet> $$1, HolderGetter<PlacedFeature> $$2) {
        HolderSet.Direct $$3 = HolderSet.direct($$1.getOrThrow(BuiltinStructureSets.STRONGHOLDS), $$1.getOrThrow(BuiltinStructureSets.VILLAGES));
        FlatLevelGeneratorSettings $$4 = new FlatLevelGeneratorSettings((Optional<HolderSet<StructureSet>>)Optional.of($$3), FlatLevelGeneratorSettings.getDefaultBiome($$0), FlatLevelGeneratorSettings.createLakesList($$2));
        $$4.getLayersInfo().add((Object)new FlatLayerInfo(1, Blocks.BEDROCK));
        $$4.getLayersInfo().add((Object)new FlatLayerInfo(2, Blocks.DIRT));
        $$4.getLayersInfo().add((Object)new FlatLayerInfo(1, Blocks.GRASS_BLOCK));
        $$4.updateLayers();
        return $$4;
    }
