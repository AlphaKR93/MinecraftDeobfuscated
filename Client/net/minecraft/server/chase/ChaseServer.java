...
public class ChaseServer {
    ...
    public void start() throws IOException {
        ...
        this.serverSocket = new ServerSocket(this.serverPort, 50, InetAddress.getByName((String)this.serverBindAddress));
                    ...
                    for (Socket $$3 : this.clientSockets) {
                        ...
                        Util.ioPool().submit(() -> {
                            try {
                                ...
                                LOGGER.info("Remote control client socket got an IO exception and will be closed", (Throwable)$$3);
                                IOUtils.closeQuietly((Socket)$$3);
                            }
                        });
                    }
                }
                List $$4 = (List)this.clientSockets.stream().filter(Socket::isClosed).collect(Collectors.toList());
                this.clientSockets.removeAll((Collection)$$4);
            }
    ...
    private void runAcceptor() {
        try {
            while (this.wantsToRun) {
                ...
                this.clientSockets.add((Object)$$0);
            }
        }
            ...
            if (this.wantsToRun) {
                LOGGER.error("Remote control server closed because of an IO exception", (Throwable)$$2);
            }
        }
        finally {
            IOUtils.closeQuietly((ServerSocket)this.serverSocket);
    ...
    private PlayerPosition getPlayerPosition() {
        ...
        String $$2 = (String)ChaseCommand.DIMENSION_NAMES.inverse().get($$1.getLevel().dimension());
        if ($$2 == null) {
            return null;
        }
    ...
    record PlayerPosition(String dimensionName, double x, double y, double z, float yRot, float xRot) {
        String format() {
            return String.format((Locale)Locale.ROOT, (String)"t %s %.2f %.2f %.2f %.2f %.2f\n", (Object[])new Object[]{this.dimensionName, this.x, this.y, this.z, Float.valueOf((float)this.yRot), Float.valueOf((float)this.xRot)});
        }
    }
}