...
public class LocateCommand {
    ...
    private static final DynamicCommandExceptionType ERROR_STRUCTURE_NOT_FOUND = new DynamicCommandExceptionType($$0 -> Component.translatable("commands.locate.structure.not_found", $$0));
    private static final DynamicCommandExceptionType ERROR_STRUCTURE_INVALID = new DynamicCommandExceptionType($$0 -> Component.translatable("commands.locate.structure.invalid", $$0));
    private static final DynamicCommandExceptionType ERROR_BIOME_NOT_FOUND = new DynamicCommandExceptionType($$0 -> Component.translatable("commands.locate.biome.not_found", $$0));
    private static final DynamicCommandExceptionType ERROR_POI_NOT_FOUND = new DynamicCommandExceptionType($$0 -> Component.translatable("commands.locate.poi.not_found", $$0));
    ...
    private static Optional<? extends HolderSet.ListBacked<Structure>> getHolders(ResourceOrTagKeyArgument.Result<Structure> $$0, Registry<Structure> $$12) {
        return (Optional)$$0.unwrap().map($$1 -> $$12.getHolder((ResourceKey<Structure>)$$1).map($$0 -> HolderSet.direct($$0)), $$12::getTag);
    }
    ...
    private static int locateStructure(CommandSourceStack $$0, ResourceOrTagKeyArgument.Result<Structure> $$1) throws CommandSyntaxException {
        Registry<Structure> $$2 = $$0.getLevel().registryAccess().registryOrThrow(Registries.STRUCTURE);
        HolderSet $$3 = (HolderSet)LocateCommand.getHolders($$1, $$2).orElseThrow(() -> ERROR_STRUCTURE_INVALID.create((Object)$$1.asPrintable()));
        BlockPos $$4 = new BlockPos($$0.getPosition());
        ServerLevel $$5 = $$0.getLevel();
        Stopwatch $$6 = Stopwatch.createStarted((Ticker)Util.TICKER);
    ...
    private static int locateBiome(CommandSourceStack $$0, ResourceOrTagArgument.Result<Biome> $$1) throws CommandSyntaxException {
        BlockPos $$2 = new BlockPos($$0.getPosition());
        Stopwatch $$3 = Stopwatch.createStarted((Ticker)Util.TICKER);
    ...
    private static int locatePoi(CommandSourceStack $$0, ResourceOrTagArgument.Result<PoiType> $$1) throws CommandSyntaxException {
        ...
        Stopwatch $$4 = Stopwatch.createStarted((Ticker)Util.TICKER);
        ...
        return LocateCommand.showLocateResult($$0, $$1, $$2, ((Pair)$$5.get()).swap(), "commands.locate.poi.success", false, $$4.elapsed());
    }

    private static String getElementName(Pair<BlockPos, ? extends Holder<?>> $$02) {
        return (String)((Holder)$$02.getSecond()).unwrapKey().map($$0 -> $$0.location().toString()).orElse((Object)"[unregistered]");
    }
    ...
    public static int showLocateResult(CommandSourceStack $$0, ResourceOrTagArgument.Result<?> $$12, BlockPos $$22, Pair<BlockPos, ? extends Holder<?>> $$3, String $$4, boolean $$5, Duration $$6) {
        String $$7 = (String)$$12.unwrap().map($$1 -> $$12.asPrintable(), $$2 -> $$12.asPrintable() + " (" + LocateCommand.getElementName($$3) + ")");
    ...
    public static int showLocateResult(CommandSourceStack $$02, ResourceOrTagKeyArgument.Result<?> $$12, BlockPos $$2, Pair<BlockPos, ? extends Holder<?>> $$3, String $$4, boolean $$5, Duration $$6) {
        String $$7 = (String)$$12.unwrap().map($$0 -> $$0.location().toString(), $$1 -> "#" + $$1.location() + " (" + LocateCommand.getElementName($$3) + ")");
    ...
    private static int showLocateResult(CommandSourceStack $$0, BlockPos $$1, Pair<BlockPos, ? extends Holder<?>> $$22, String $$3, boolean $$4, String $$5, Duration $$6) {
        ...
        String $$9 = $$4 ? String.valueOf((int)$$7.getY()) : "~";
        MutableComponent $$10 = ComponentUtils.wrapInSquareBrackets(Component.translatable("chat.coordinates", $$7.getX(), $$9, $$7.getZ())).withStyle((UnaryOperator<Style>)((UnaryOperator)$$2 -> $$2.withColor(ChatFormatting.GREEN).withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/tp @s " + $$7.getX() + " " + $$9 + " " + $$7.getZ())).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("chat.coordinates.tooltip")))));
