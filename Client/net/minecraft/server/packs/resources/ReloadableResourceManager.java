...
AutoCloseable {
    ...
    public ReloadableResourceManager(PackType $$0) {
        this.type = $$0;
        this.resources = new MultiPackResourceManager($$0, (List<PackResources>)List.of());
    }
    ...
    public ReloadInstance createReload(Executor $$0, Executor $$1, CompletableFuture<Unit> $$2, List<PackResources> $$3) {
        LOGGER.info("Reloading ResourceManager: {}", LogUtils.defer(() -> $$3.stream().map(PackResources::packId).collect(Collectors.joining((CharSequence)", "))));
        this.resources.close();
