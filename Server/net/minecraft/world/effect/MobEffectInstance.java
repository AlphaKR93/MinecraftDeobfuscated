...
implements Comparable<MobEffectInstance> {
    ...
    public String toString() {
        String $$1;
        if (this.amplifier > 0) {
            String $$0 = this.getDescriptionId() + " x " + (this.amplifier + 1) + ", Duration: " + this.duration;
        } else {
            $$1 = this.getDescriptionId() + ", Duration: " + this.duration;
        }
        if (!this.visible) {
            $$1 = $$1 + ", Particles: false";
        }
    ...
    public int compareTo(MobEffectInstance $$0) {
        ...
        return ComparisonChain.start().compare(Boolean.valueOf((boolean)this.isAmbient()), Boolean.valueOf((boolean)$$0.isAmbient())).compare(this.getDuration(), $$0.getDuration()).compare(this.getEffect().getColor(), $$0.getEffect().getColor()).result();
    }

    public static class FactorData {
        public static final Codec<FactorData> CODEC = RecordCodecBuilder.create($$02 -> $$02.group((App)ExtraCodecs.NON_NEGATIVE_INT.fieldOf("padding_duration").forGetter($$0 -> $$0.paddingDuration), (App)Codec.FLOAT.fieldOf("factor_start").orElse((Object)Float.valueOf((float)0.0f)).forGetter($$0 -> Float.valueOf((float)$$0.factorStart)), (App)Codec.FLOAT.fieldOf("factor_target").orElse((Object)Float.valueOf((float)1.0f)).forGetter($$0 -> Float.valueOf((float)$$0.factorTarget)), (App)Codec.FLOAT.fieldOf("factor_current").orElse((Object)Float.valueOf((float)0.0f)).forGetter($$0 -> Float.valueOf((float)$$0.factorCurrent)), (App)ExtraCodecs.NON_NEGATIVE_INT.fieldOf("effect_changed_timestamp").orElse((Object)0).forGetter($$0 -> $$0.effectChangedTimestamp), (App)Codec.FLOAT.fieldOf("factor_previous_frame").orElse((Object)Float.valueOf((float)0.0f)).forGetter($$0 -> Float.valueOf((float)$$0.factorPreviousFrame)), (App)Codec.BOOL.fieldOf("had_effect_last_tick").orElse((Object)false).forGetter($$0 -> $$0.hadEffectLastTick)).apply((Applicative)$$02, FactorData::new));
        ...
        public void update(MobEffectInstance $$0) {
            boolean $$1;
            this.factorPreviousFrame = this.factorCurrent;
            boolean bl = $$1 = $$0.duration > this.paddingDuration;
            if (this.hadEffectLastTick != $$1) {
                this.hadEffectLastTick = $$1;
                this.effectChangedTimestamp = $$0.duration;
                this.factorStart = this.factorCurrent;
                this.factorTarget = $$1 ? 1.0f : 0.0f;
            }
            float $$2 = Mth.clamp(((float)this.effectChangedTimestamp - (float)$$0.duration) / (float)this.paddingDuration, 0.0f, 1.0f);
